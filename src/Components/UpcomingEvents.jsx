  ...
  useEffect(() => {
    if (
      ...
      const filteredEvents = [4, 1, 2]
        .flatMap((type) => allNewEvents?.filter(e => e.event_type == type).slice(0, 4));
        ...
        <div className=" px-[1px]">
          ...
          <div className="block lg:hidden">
            <div className="flex justify-between items-center text-xs pb-[2px]">
              <div className="flex gap-[2px] justify-between items-center text-[var(--theme1-bg)] pt-[1px] px-1">
                <span className="flex justify-center items-center py-1 px-2 border border-[var(--theme1-bg)] rounded-xl">
                  - LIVE
                </span>
                <span className="flex justify-center items-center py-1 px-2 border border-[var(--theme1-bg)] rounded-xl">
                  {" "}
              ...
              <div className="flex justify-center items-center">
                ...
                <select name="" id="" className="bg-[var(--theme1-bg)] text-[13px] text-[var(--secondary-color)] rounded-sm ml-1 p-[2px]">
                      ...
                      <div className=" px-1 pb-1">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex justify-center items-center gap-1">
                            ...
                            {item.is_bookmaker && item.is_bookmaker == "True" &&
                              <span className="mr-1 text-xs font-semibold">
                                <img src="/Images/ic_bm.webp" alt="" className="w-4" />
                              </span>}
                            {item.is_fancy && item.is_fancy == "True" &&
                              <span className="mr-2 text-xs font-semibold">
                                <img src="/Images/ic_fancy.webp" alt="" className="w-4" />
                              </span>}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-around items-center text-xs font-bold leading-[0.6rem] mt-[0.5rem]">
                          <span>1</span>
                          <span>X</span>
                          <span>2</span>
                        </div>
          ...
          {/* For Pc */}
          <div className="hidden lg:block text-[#121416]">
            <div className="flex justify-end items-center text-xs p-1 border-b border-gray-300">
              <div className="flex gap-[2px] justify-between items-center text-xs text-[var(--theme1-bg)] pt-[1px] px-1 mr-8">
                <span className="flex justify-center items-center py-1 px-2 border border-[var(--theme1-bg)] rounded-xl">
                  - LIVE
                </span>
                <span className="flex justify-center items-center py-1 px-2 border border-[var(--theme1-bg)] rounded-xl">
              ...
              <div className="flex justify-center items-center">
                <span className="font-bold">View by:</span>
                <select name="" id="" className="bg-[var(--theme1-bg)] text-[13px] text-[var(--secondary-color)] ml-1 p-1">
                  <option value="TIME">TIME</option>
                  <option value="COMPITITION">COMPITITION</option>
                </select>
              </div>
            </div>
            <div className="flex justify-between items-center w-full text-xs font-semibold p-1 border-b-2 border-gray-300">
              <span className="pl-1 w-[60%]">Game</span>
            ...
            <div className={`${sportType != 'sports' ? "max-h-[16rem] lg:max-h-full" : "h-auto"} overflow-scroll scroll-hide`}>
              <ul>
                {events?.length > 0 &&
                  events?.map((item, index) => (
                    <li className=" flex w-full border-b border-[#d6d7d8]" key={index}>
                      <div className=" px-2 w-[60%]">
                        <div className="flex justify-between items-center">
                          <Link to={`/matchupdates/${item.event_id}/${item.is_inplay === "True" ? "Inplay" : "Going Inplay"}`}>
                            <span className="text-[13px] whitespace-nowrap text-ellipsis overflow-hidden tracking-[0.04rem]">
                              {item.event_name} / <span className="">{item.open_date}</span>
                            </span>
                          </Link>
                          <div className="flex justify-center items-center gap-1">
                            {item.is_inplay == "True" && (
                      ...
                      <th className="flex justify-between item-center text-center text-[13px] font-bold w-[40%]">
                        <td className="bg-[#72bbef] p-1 w-[100%]">
                          3.9
                        </td>
                        <td className="bg-[#faa9ba] p-1 w-[100%]">
