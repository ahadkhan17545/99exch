...
function ProfitLossEvents(props) {
    ...
    const { from_date, to_date, event_type, event_id, is_casino, get_user_id } = location.state || {};
    ...
    const navigateToDetails = (record) => {
        if (get_user_id) {
            ...
            navigate(`/reports/event-market-user-wise-pl`, { state: { from_date: formData.from_date, to_date: formData.to_date, event_type: record?.event_type, is_casino: record?.is_casino, event_id: record?.match_id, market_id: record?.market_id, market_name: record?.market_name, betting_type: record?.betting_type, selection_id: record?.selection_id, get_user_id: get_user_id ? get_user_id : '' } });
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className='font-extrabold text-[#212529] text-[0.813rem] '>
                    {record?.result_name ? record?.result_name : 0}
                </div>
            ),
        },
        ...
        //             // record?.updated_at ? moment(record?.updated_at).format('YYYY-MM-DD hh:mm:ss a') : ''
        //         }</span>
        //     )
        // }
    ];
    ...
    const filterProfitLossByEvents = async (e) => {
        ...
        var config = {
            method: 'post',
            url: `${Appconfig.apiUrl}reports/auraEventMarketWisePLNew`,
            headers: {
                                        ...
                                        <div className='col-span-12 lg:col-span-6 flex items-center justify-center lg:justify-end'>
                                            <div className='text-[#333]'>
                                                <label>
                                                    Search:
                                                    <Input
                                                        onChange={e => handleSearch(e.target.value, ['event_type_name', 'event_name', 'market_name'])}
