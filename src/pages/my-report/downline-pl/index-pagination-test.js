...
function DownlinePL() {
    ...
    const { from_date, to_date, user_id } = location.state || {};

    const [formData, setFormData] = useState({
        "user_id": user_id ? user_id : userInfo && userInfo?._id,
        "from_date": from_date ? from_date : moment().format('YYYY-MM-DD'),
    ...
    useEffect(() => {
        // Check if location.state is not null and update formData
        if (location.state) {
            setFormData(prevFormData => ({
                ...prevFormData,
                from_date: location.state.from_date || prevFormData.from_date,
                to_date: location.state.to_date || prevFormData.to_date,
                user_id: location.state.user_id || prevFormData.user_id,
            }));
            setLoading(true);
        }
    }, [location.state]);
    ...
    //         navigate(`/reports/downline-pl`, { state: { from_date: formData.from_date, to_date: formData.to_date, user_id: record._id } });
    //     } else {
    //         navigate(`/my-account`, { state: { from_date: formData.from_date, to_date: formData.to_date, id: record._id, activeTab: 'profit-loss' } });
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                record.user_type != "User" ?
                    (
                        <Link className='font-extrabold text-[#2789ce] text-[0.813rem] '
                            to={'/reports/downline-pl'} state={{ from_date: formData.from_date, to_date: formData.to_date, user_id: record._id.x }}
                        >
                            {record?.user_name}
                        </Link>
                    )
                    ...
                    (
                        <Link className='font-extrabold text-[#2789ce] text-[0.813rem] '
                            to={'/my-account'} state={{ id: record._id, activeTab: 'profit-loss' }}
                        >
                            {record?.user_name}
                        </Link>
                    )
        ...
        },
        {
            ...
            sorter: false,
            render: (_, record) => (
                <span className='font-extrabold text-[#212529] text-[0.813rem] '>{record?.commissionAmount?.toFixed(2)}</span>
            )
        },
    ...
    const [current, setCurrent] = useState(1); // skip
    const [pageSize, setPageSize] = useState(10); // take
    ...
    const filterDownlinePL = async (current = null) => {
        ...
        var config = {
            method: 'post',
            url: `${Appconfig.apiUrl}reports/auraDownlinePLPaginate/${current}/${pageSize}`,
            headers: {
        ...
        axios(config)
            .then(function (response) {
                try {
                    // console.log('response.data.resultData', response.data);
                    ...
                    <div className='lg:mb-[24px] rounded-[.25rem]  lg:pb-[0] w-full bg-[#e0e6e6]  text-[#212529] [word-wrap:break-word] border-[1px] border-[#000501] '>
                        <div className='!p-0 flex-auto'>
                            <div className='flex flex-wrap pt-[10px]'>
                                ...
                                <div className='px-[10px] mb-[1rem] flex items-center  justify-between lg:!mb-[10px] flex-[0_0_auto] w-[50%] lg:w-[16.66%]'>
                                    <button onClick={() => {
                                        filterDownlinePL()
                                    ...
                                    <button type="button" className="mt-[1rem] lg:mt-[0px] font-bold !px-[10px] !py-[5px] !text-[.875rem] text-[#ffffff] bg-[linear-gradient(-180deg,_#2E4B5E_0%,_#243A48_82%)] border-[#cccccc] inline-block leading-normal text-center rounded-[.375rem]">
                                        <strong >Reset</strong>
                                    </button>
                                </div>
                            </div>
