...
function ProfitLossEvents(props) {
    ...
    const { from_date, to_date, event_type, is_casino, get_user_id } = location.state || {};
    ...
    const navigateToDetails = (record) => {
        if (event_type == '777') {
            navigate(`/reports/event-wise-profit-loss-bet-history`, { state: { from_date: formData.from_date, to_date: formData.to_date, event_type: record?.event_type, market_id: record?.market_id, user_idd: record?.user_id } });
        } else {
            navigate(`/reports/event-market-wise-pl/${record?.match_id}`, { state: { from_date: formData.from_date, to_date: formData.to_date, event_type: event_type, event_id: record?.match_id, is_casino: is_casino, get_user_id: get_user_id ? get_user_id : '' } });
        }
    };
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                <a href="javascript:void(0);" onClick={() => navigateToDetails(record)} className='font-extrabold text-[#2789ce] text-[0.813rem]'>
        ...
        //                 {record?.commissionAmount ? record?.commissionAmount?.toFixed(2) : 0}
        //             </span>
        //         )
        //     },
        //     {
    ...
    const filterProfitLossByEvents = async (e) => {
        ...
        var config = {
            method: 'post',
            url: `${Appconfig.apiUrl}reports/auraEventWisePLNew`,
            headers: {
                                        ...
                                        <div className='col-span-12 lg:col-span-6 flex items-center justify-center lg:justify-end'>
                                            <div className='text-[#333]'>
                                                <label>
                                                    Search:
                                                    <Input
                                                        onChange={e => handleSearch(e.target.value, ['event_type_name', 'event_name'])}
