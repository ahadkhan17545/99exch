...
function ProfitLoss({ user_id }) {
    ...
    const navigateToDetails = (event_type, is_casino) => {
        if (is_casino == "Yes") {
            navigate(`/reports/event-wise-pl/${is_casino}`, { state: { from_date: formData.from_date, to_date: formData.to_date, is_casino: is_casino,get_user_id : user_id ? user_id : userInfo?._id} });

        } else {
            navigate(`/reports/event-wise-pl/${event_type}`, { state: { from_date: formData.from_date, to_date: formData.to_date, event_type: event_type, get_user_id : user_id ? user_id : userInfo?._id } });
        }
    };
