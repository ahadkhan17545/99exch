...
function WithdrawRequest() {
    ...
    function confirmWithdrawRequest(record, status) {
        setButton(true)
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <span className='font-extrabold text-[#212529] text-[0.813rem] ml-[5px]'>
                    {record?.bank_name ? record?.bank_name : 'N/A'}
                </span>
            ),
        },
        {
            ...
            render: (_, record) => (
                <span className='font-extrabold text-[#212529] text-[0.813rem] ml-[5px]'>
                    {record?.upi_id ? record?.upi_id : record?.account_no}
                </span>
            ),
        },
        {
            ...
            render: (_, record) => (
                <span className='font-extrabold text-[#212529] text-[0.813rem] ml-[5px]'>
                    {record?.ifsc_code ? record?.ifsc_code : 'N/A'}
                </span>
            ),
        },
        {
            ...
            render: (_, record) => (
                <span className='font-extrabold text-[#212529] text-[0.813rem] ml-[5px]'>
                    {record?.holder_name ? record?.holder_name : 'N/A'}
                </span>
            ),
        },
        {
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className='flex items-center gap-x-4' >
                    <button disabled={button} onClick={() => confirmWithdrawRequest(record, 'Confirm')} className='p-[10px] text-[#fff] h-[30px] ml-[12px] flex bg-[#010203] rounded-[6px] border-[1px] border-[solid] border-[#010203] float-right cursor-pointer items-center justify-center'>
                        <span>Confirm</span>
                    </button>
                    <button disabled={button} onClick={() => confirmWithdrawRequest(record, 'Reject')} className='p-[10px] text-[#fff] h-[30px] ml-[12px]  flex bg-[#dc3545] rounded-[6px] border-[1px] border-[solid] border-[#dc3545] float-right cursor-pointer items-center justify-center'>
                        Reject
                    </button>
                </div>
            )
    ...
    const getWithdrawRequest = () => {
        ...
        var config = {
            method: "post",
            url: `${Appconfig.apiUrl}withdraw/getWithdrawRequest`,
            headers: {
                "Content-Type": "application/json",
            },
            data: data,
        ...
        axios(config)
            .then(function (response) {
                if (response.data.result) {
                    setWithdrawRequest(response.data.resultData);
                } else {
                    NotificationManager.error(response.data.resultMessage, '', 3000);
                }
            })
                    ...
                    <div className='lg:block w-full lg:w-[40vw] '>
                        <div className='grid grid-cols-12'>
                            <div className='col-span-12 px-[6px] py-[5px] font-bold text-[0.875rem] rounded-[1px] h-[35px] bg-[linear-gradient(-180deg,_#2E4B5E_0%,_#243A48_82%)] text-[#ffffff] flex flex-shrink-0 items-center justify-between'>
                                <h5 className='text-[0.938rem] font-bold mb-0 leading-normal'>Withdraw Request</h5>
                            </div>
